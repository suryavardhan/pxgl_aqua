...
if(class_exists('AQ_Page_Builder')) {
	...
	*******************/
	 ...
	 */
	add_action ('wp','pxglm_page_builder_pluggable', 10);
	function pxglm_page_builder_pluggable(){ 
		 ...
		 */
		if(!( function_exists('pxglm_before_block') )){
			function pxglm_before_block($instance){
				extract($instance);
				...
				echo '<div id="aq-block-'.$template_id.'-'.$number.'" class="aq-block aq-block-'.$id_base.' col-md-'.$size.' '.$column_class.' '. $clearfix.'">';
			}
		}
		 ...
		 */
		if(!( function_exists('pxglm_after_block') )){
			function pxglm_after_block($instance){
				extract($instance);
				echo '</div>';
			}
		}
